...
const EXPECTED_ERROR_MESSAGE = "Alias 'WHERE' in Column is a reserved SQLite keyword";

describe('Validate Reserved Keyword Column Alias', () => {

  test('Reserved keyword as column alias fails', () => {
    const query = SelectQuery.create()
      .from(new TableFrom('users'))
      .column(new Alias(new Column('id'), 'WHERE'));
  ...
  test('Reserved keyword as column alias fails using Shorthand API', () => {
    const query = SELECT(FROM('users'), COLUMN('id').as('WHERE'));
