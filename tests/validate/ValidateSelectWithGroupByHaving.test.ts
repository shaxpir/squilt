...
describe('Validate Select Query with GROUP BY and HAVING', () => {

  test('valid query with GROUP BY and HAVING passes both validators', () => {
  ...
  test('valid query with GROUP BY and HAVING passes both validators using Shorthand API', () => {
    const query = SELECT(
      FROM('orders'), 
      COLUMN('user_id'),
