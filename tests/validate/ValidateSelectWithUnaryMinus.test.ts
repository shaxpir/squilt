...
describe('Validate Select Query with Unary MINUS', () => {

  test('valid query with unary MINUS passes both validators', () => {
  ...
  test('valid query with unary MINUS passes both validators using Shorthand API', () => {
    const query = SELECT(
      FROM('products').as('p'),
      MINUS(COLUMN('p', 'price')).as('negated_price')
    );
